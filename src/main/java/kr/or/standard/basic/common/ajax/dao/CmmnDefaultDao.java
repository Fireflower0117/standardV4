...
public class CmmnDefaultDao {
    ...
    public List<? extends CmmnDefaultVO> selectList(String qid, CommonMap hashMap) {
        ...
        log.info("[DEFAULT DAO-SELECT] qid : {} , data : {}", qid, hashMap);
    ...
    public int insert(String qid, CmmnDefaultVO vo) {
        log.info("[DEFAULT DAO-INSERT DEFAULT] qid : {} , vo : {}", qid, vo);
    ...
    public int insert(String qid, List<? extends CmmnDefaultVO> voList) {
        log.info("[DEFAULT DAO-INSERT-LIST DEFAULT] {}", qid );
        int effRows = 0;
        for (CmmnDefaultVO item : voList) {
            effRows += insert(qid, item);
        }
        log.info("[DEFAULT DAO-INSERT-LIST DEFAULT COMPLETE] qid : {}, effRows : {}", qid, effRows );
        return effRows;
    }


    public int insertList(String qid, List<? extends CmmnDefaultVO> voList) {
        log.info("[DEFAULT DAO-INSERTLIST DEFAULT] qid : {} , size : {} ", qid , voList.size() );
        int effRows = sqlsession.insert(qid, voList);
        log.info("[DEFAULT DAO-INSERTLIST DEFAULT COMPLETE] qid : {}, effRows : {}", qid, effRows );
        return effRows;
    }

    public boolean batchInsert(String qid, List<? extends CmmnDefaultVO> voList) {
        SqlSession session = sqlSessionFactory.openSession(ExecutorType.BATCH, false );
        boolean isResult = false;
        try{
            session.insert(qid, voList);
    ...
    public int update(String qid, CmmnDefaultVO vo) {
        log.info("[DEFAULT DAO-UPDATE DEFAULT] qid : {} , vo : {}", qid, vo);
    ...
    public int update(String qid, List<? extends CmmnDefaultVO> volist) {
        log.info("[DEFAULT DAO-UPDATE-LIST DEFAULT] qid : {}", qid );
        int effRows = 0;
        for (CmmnDefaultVO vo : volist) {
            effRows += update(qid, vo);
        }
        log.info("[DEFAULT DAO-UPDATE-LIST DEFAULT COMPLETE] qid : {} , effRows : {}", qid, effRows);
        return effRows;
    }

    public int update(String qid, String str) {
        log.info("[DEFAULT DAO-UPDATE DEFAULT] qid : {} , str : {}", qid, str);
        int effCnt = sqlsession.update(qid, str);
        log.info("[DEFAULT DAO-UPDATE COMPLETE] qid : {} , effCnt : {}", qid, effCnt);
        return effCnt;
    }

    public boolean batchUpdate(String qid, List<? extends CmmnDefaultVO> voList) {
        SqlSession session = sqlSessionFactory.openSession(ExecutorType.BATCH, false );
        boolean isResult = false;
        try{
            session.update(qid, voList);
            session.flushStatements();
            session.commit();
            isResult = true;
        }
    ...
    public int delete(String qid, CmmnDefaultVO vo) {
        log.info("[DEFAULT DAO-DELETE DEFAULT] qid : {} , vo : {}", qid, vo);
        int effCnt = sqlsession.delete(qid, vo);
        log.info("[DEFAULT DAO-DELETE COMPLETE] qid : {} , effCnt : {}", qid, effCnt);
        vo.setEffCnt(effCnt);
        return effCnt;
    }

    public int delete(String qid, List<? extends CmmnDefaultVO> volist) {
        log.info("[DEFAULT DAO-DELETE-LIST DEFAULT] qid : {}", qid );
        int effRows = 0;
        for (CmmnDefaultVO vo : volist) {
            effRows += delete(qid, vo);
        }
        log.info("[DEFAULT DAO-DELETE-LIST DEFAULT COMPLETE] qid : {} , effRows : {}", qid, effRows);
        return effRows;
    }

    public int delete(String qid, String str) {
        log.info("[DEFAULT DAO-DELETE DEFAULT] qid : {} , str : {}", qid, str);
        int effCnt = sqlsession.delete(qid, str);
        log.info("[DEFAULT DAO-DELETE COMPLETE] qid : {} , effCnt : {}", qid, effCnt);
        return effCnt;
    }

    public boolean batchDelete(String qid, List<? extends CmmnDefaultVO> voList) {
        SqlSession session = sqlSessionFactory.openSession(ExecutorType.BATCH, false );
        boolean isResult = false;
        try{
            session.delete(qid, voList);
            session.flushStatements();
            session.commit();
            isResult = true;
        }
