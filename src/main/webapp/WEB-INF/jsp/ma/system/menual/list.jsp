...
$(document).ready(function(){
     ...
     // 이 메뉴의 기능이 사용자메뉴얼, 운영자메뉴얼같은 파일을 관리하는 게시판형태인지.??
     // 특정페이지의 '?' 같은 버튼의 페이지 사용메뉴얼을 GUI형태 팝업으로 제공하는 기능인지???
     /**************************************************************************
     ***********************        페이지 세팅            ***********************
     ***************************************************************************/
    ...
    // 검색조건 구분
     on.html.dynaGenSelectOptions({ targetInfo    : { targetId : "#searchKeyCd" }
                                  ...
                                  , dataInfo      : [ { code : "menualNm"  , text:"메뉴구분명" }
                                                    , { code : "userNm"    , text:"담당자명" }
                                                    ]
                                 });
     // Paging Per Row
      ...
      ***************************************************************************/
      ...
      function searchData() {
        ...
        var displayInfoObj = {
          ...
          , displayColInfos : {
                normal_tab_columns : [
                        {id: "rowindx"   , colType: "number" , data_id1:"mnlSerno", classNm: "number"}
                      , {id: "menuClNm"  , colType: "td"  , data_id1:"mnlSerno"}
                      , {id: "menuExpl"  , colType: "td"  , data_id1:"mnlSerno"}
                      , {id: "tchgrNm"   , colType: "td"  , data_id1:"mnlSerno"}
                      , {id: "regDt"     , colType: "td"  , data_id1:"mnlSerno" , classNm: "date"}
                ]
          }
        }
     ...
     // 검색 초기화
      ...
      $(".board_list > tbody").on("click" , (evt) => {
        ...
        if(!on.valid.isEmpty(selMenualSerno)){
              on.html.dynaGenHiddenForm({ formDefine : { fid       : "authViewForm", action: "/ma/system/menual/view.do", method: "post", isSubmit : true }
       ...
       <c:if test="${USER_AUTH.WRITE_YN== 'Y'}">
             ...
             $("#btnWrite").on("click", (evt) => {
                 on.html.dynaGenHiddenForm({ formDefine : { fid : "authWriteForm", action: "/ma/system/menual/regist.do", method: "post", isSubmit : true }
                                           , formAttrs : [ { name : "searchCondition" , value : JSON.stringify(searchConditionObj) } ]
                 });
             });
       </c:if>
...
<div class="tbl">
    ...
    <div class="paging_wrap">
        ...
        <c:if test="${USER_AUTH.WRITE_YN== 'Y'}">
			<div class="btn_right">
                <button id="btnWrite" class="btn blue btn_write">등록</button>
            </div>
		</c:if>
    </div>
</div>