	                            ...
	                            <c:choose>
	                                <c:when test="${not empty item.imgList }">
	                                    <c:forEach var="img" items="${item.imgList }">
	                                        <img src="/file/getImage.do?atchFileId=<c:out value="${item.atchFileId }"/>&fileSn=<c:out value="${img.fileSn}"/>&fileNmPhclFileNm=<c:out value="${img.fileNmPhclFileNm}"/>" id="viewImage" alt="항목 이미지" width="100%">
	                                    </c:forEach>
	                                </c:when>
	                                <c:otherwise>
	                                    파일이 없습니다.
