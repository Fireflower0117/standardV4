	...
	<table class="board_view">
		...
		<tbody>
			...
			<tr>
				...
				<td><c:out value="${bltnbVO.ntiYn eq 'Y' ? '공지' : '미공지'}"/></td>
				<th scope="row">공지기간</th>
				<td>
					<c:choose>
						<c:when test="${not empty bltnbVO.ntiStrtDt and not empty bltnbVO.ntiEndDt}">
							<c:out value="${bltnbVO.ntiStrtDt}"/>&nbsp;~&nbsp;<c:out value="${bltnbVO.ntiEndDt}"/>
						</c:when>
						<c:when test="${not empty bltnbVO.ntiStrtDt}">
							<c:out value="${bltnbVO.ntiStrtDt}"/>
						</c:when>
						<c:when test="${not empty bltnbVO.ntiEndDt}">
							<c:out value="${bltnbVO.ntiEndDt}"/>
						</c:when>
						<c:otherwise>
							-
						</c:otherwise>
