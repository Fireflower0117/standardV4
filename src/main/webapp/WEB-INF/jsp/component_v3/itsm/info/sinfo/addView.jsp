...
function fncFileShow(fileId, seqo, nm){
	fncAjax('largeImg.do', {"atchFileId" : fileId, "fileSn" : seqo, "fileRlNm" : nm}, 'html', true, '', function(data){
								...
								<c:if test="${fn:length(fileList) gt 0}">
									<c:forEach var="file" items="${fileList}" varStatus="fileStatus">
										<c:if test="${fn:contains(file.fileTpNm, 'image') }">
											<li>
												<a href="#" onclick="fncFileShow('${file.atchFileId}', '${file.fileSn}','${file.fileRlNm}');return false; ">
													<figure class="thum_img">
														<img src='/itsm/file/getByteImage.do?atchFileId=${file.atchFileId}&fileSn=${file.fileSn}' alt="image">
