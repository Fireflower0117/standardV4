...
<script>
    $(document).ready(() => {
        ...
        ********************      페이지 기본정보 조회 / 세팅       ********************
        ***************************************************************************/
        // Page내부 사용할 공통코드 일괄조회 (1 Transaction)
        ...
        ***************************************************************************/
        // 게시판 코드 중복여부
        let isBoardTypeDuplPass = false;
        $("#boardCd").on("keypress", (evt) => { // 중복 체크했어도 수정하면 다시 갱신해야함..
            isBoardTypeDuplPass = false;
        });
        // 게시판코드 중복 체크
        $("#btnBoardCdDupl").on("click", (evt) => {

            // 게시판 중복체크 유효성 검증 대상
            let boardTypeCdValidList  = [ {name : "boardCd"      , label : "게시판코드"           ,  rule: {required:true, minlength : 3} }
                                          ];

            on.xhr.ajax({ cmd : "selectOne" ,sql : "on.standard.system.boardtype.inqBoardCdCount"
                        , boardCd :  on.html.getEleVal({ele : "#boardCd" })
                        , validation : { formId : "#boardTypeForm" , validationList : boardTypeCdValidList  }  // 유효성검증 기능 포함
                        , successFn : function (rs){
                               if("notDupl" === rs.isBoardTypeDupl){
                                   if( on.msg.confirm({message : "사용가능한 게시판코드입니다.\n사용하시겠습니까?"}) == true){
                                    isBoardTypeDuplPass = true;
                                   }
                               }
                            }
                        , failFn    : function (err){
        ...
        <c:if test="${USER_AUTH.UPDATE_YN== 'Y'}">
            ...
            $("#btnRegist").on("click" , (evt) => {
                on.xhr.ajax({ cmd: "insert" , sql : "on.standard.system.boardtype.mergeInsBoardType"
                            , boardCd :  on.html.getEleVal({ele : "#boardCd" })
                            ...
                            , successFn : function (rs){
                                   if(rs[0].execResult === "success"){
                                      on.msg.showMsg({message : "게시판유형이 등록되었습니다."});
                                      goList();
                                   }
                                }
                            , failFn    : function (err){
...
<form id="boardTypeForm" name="boardTypeForm">
  <table class="board_write">
    ...
    <tr>
      ...
      <td class="c" >
          <input type="text" id="boardCd" name="boardCd" class="w30p" title="게시판코드" maxlength="30"/>
          <input type="button" id="btnBoardCdDupl" value="중복검사" ></input>
      </td>
