...
<div class="board_top">
    ...
    <!-- <div class="board_right">
        <button class="btn btn_excel">엑셀 다운로드</button>
    </div> -->
</div>
			...
			<c:when test="${fn:length(resultList) > 0 }">
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr data-bltnbserno="<c:out value='${result.bltnbSerno}'/>">
						...
						<td>
							<c:choose>
								<c:when test="${result.ntiCheck eq 'Y'}"> 							
									<i class="xi-volume-up"></i>						
								</c:when>
								<c:otherwise>
									<c:out value="${result.ntiYn}"/>
								</c:otherwise>
							</c:choose>
						</td>
						<td>
							<c:choose>
								<c:when test="${not empty result.ntiStrtDt and not empty result.ntiEndDt}">
									<c:out value="${result.ntiStrtDt}"/>&nbsp;~&nbsp;<c:out value="${result.ntiEndDt}"/>
								</c:when>
								<c:when test="${not empty result.ntiStrtDt}">
									<c:out value="${result.ntiStrtDt}"/>
								</c:when>
								<c:when test="${not empty result.ntiEndDt}">
									<c:out value="${result.ntiEndDt}"/>
								</c:when>
								<c:otherwise>
									-
								</c:otherwise>
