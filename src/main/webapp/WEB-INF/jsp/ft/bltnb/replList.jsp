...
$(document).ready(function(){
	...
	const fncCallReplReplList = function(index,replSerno){
		$.ajax({
			...
			,success : function(data) {
				<%-- 댓글 폼 생성 --%>
				var html = '<li class="write_box">'
                		 + 		'<textarea rows="2" placeholder="0자 이내" name="replCtt" id="reply_replCtt_' + index + '"></textarea>'
                		 + 		'<button type="button" class="btn blue btn_reply_submit" data-index="' + index + '" data-replserno="' + replSerno + '" >등록</button>'
                		 + '</li>';
                <%-- 댓글 리스트 생성 --%>
                for(var i =0; i < data.returnData.length; i++){
                	html += '<li>'
                		  ...
                		  + 	'</div>'
                	<%-- 댓글 작성자일경우만 버튼 노출 --%>
                	if("${sessionScope.ft_user_info.userSerno}" == data.returnData[i].replRegrSerno){
                		html += '<div class="btns" id="repl_btn_box_' + index + '_' + i + '" data-index="' + index + '_' + i + '" data-replserno="' + data.returnData[i].replSerno + '" data-replregrserno="' + data.returnData[i].replRegrSerno + '">'
                	...
                	html += 	'<p id="repl_ctt_' + index + '_' + i + '">' + data.returnData[i].replCtt + '</p>'
                		  + 	'<div class="write_box" id="repl_write_box_' + index + '_' + i + '" data-index="' + index + '_' + i + '" data-replserno="' + data.returnData[i].replSerno + '" data-replregrserno="' + data.returnData[i].replRegrSerno + '">'
                		  + 	'</div>'
                		  + '</li>';
	...
	<%-- 댓글작성 --%>
	$(document).on("click", ".btn_repl_submit", function(e){
		e.stopImmediatePropagation()
		<%-- 로그인 여부 판별 --%>
		if("${sessionScope.ft_user_info}"){
			<%-- 댓글이 빈 값이 아닐때만 실행 --%>
			if($("#replCtt").val() != null && $("#replCtt") != ""){
				$('textarea[name="replCtt"]').prop("disabled", true);
					$("#replCtt").prop("disabled", false);
				fncProcAjax('replInsert')
			}else{
				alert("댓글을 작성후 등록해주세요.")
				$("#replCtt").focus();
			}
		}else{
	...
	$(document).on("click", ".btn_reply_submit", function(e){
		...
		if("${sessionScope.ft_user_info}"){
			<%-- 댓글이 빈 값이 아닐때만 실행 --%>
			if($("#reply_replCtt_" + $(this).data("index")).val() != null && $("#reply_replCtt_" + $(this).data("index")).val() != ""){
				$('textarea[name="replCtt"]').prop("disabled", true);
				$("#uprReplSerno").val($(this).data("replserno"));
				$("#reply_replCtt_" + $(this).data("index")).prop("disabled", false);
				fncProcAjax('replInsert');
			}else{
				alert("댓글을 작성후 등록해주세요.")
				$("#reply_replCtt_" + $(this).data("index")).focus();
			}
		}else{
			if(confirm("로그인이 필요한 서비스입니다. 로그인 하시겠습니까?")){
				location.href = "/ft/login.do";
	...
	$(document).on("click", ".btn_repl_modify", function(e){
		e.stopImmediatePropagation()
		<%-- 작성자 여부 판별--%>
		if("${sessionScope.ft_user_info.userSerno}" == $(this).parent().data("replregrserno")){
			<%-- 댓글이 빈 값이 아닐때만 실행 --%>
...
<div class="cmnt_write">
    <div class="write_box">
        <textarea rows="3" placeholder="0자 이내" name="replCtt" id="replCtt"></textarea>
        <input type="hidden" name="replSerno" id="replSerno">
        <input type="hidden" name="uprReplSerno" id="uprReplSerno">
        <button type="button" class="btn blue btn_repl_submit" data-divn="main">등록</button>
    </div>
</div>
<div class="cmnt_list">
				 ...
				 <li>
				     <div class="reply_box">
				         <div class="reply_txt">
				             ...
				             <c:if test="${not empty sessionScope.ft_user_info and sessionScope.ft_user_info.userSerno eq result.replRegrSerno}">
				                <div class="btns" id="repl_btn_box_<c:out value='${status.index}'/>" data-index="<c:out value="${status.index}"/>" data-replserno="<c:out value="${result.replSerno}"/>" data-replregrserno="<c:out value="${result.replRegrSerno}"/>" >
				                    <button class="btn sml btn_repl_modify_form">수정</button>
				                    <button class="btn sml gray btn_repl_delete">삭제</button>
				                </div>
				             </c:if>
