package kr.or.standard.basic.component.hwp.service;
...
import kr.or.standard.basic.component.hwp.vo.CmHwpVO;
import kr.or.standard.basic.component.hwp.vo.RGBColor;
...
public class HwpService {
	...
	public final String hwpForderPath = "c:/hwpDown";
    public final String hwpBlankPath = hwpForderPath + "/sample.hwp"; 
    public final String hwpOutputPath = hwpForderPath + File.separator + "htmltohwp";
    ...
    public void run() {
        try {
            File file = new File(hwpBlankPath);
            File folder = new File(hwpForderPath);
            ...
            if (!file.exists()) {
                ...
                HWPWriter.toFile(hwpFile, hwpBlankPath);
            }


            hwpFile = HWPReader.fromFile(hwpBlankPath);
            // HTML 파일을 불러옵니다. 이 예제는 resources 폴더의 tableExample.html을 예로 합니다. 
            String html = new String(Files.readAllBytes(Paths.get(ClassLoader.getSystemResource("static/internal/standard/guide/tableExample.html").toURI())));

            // 불러온 HTML을 Jsoup로 변환합니다. 이는 각 Element를 조사하고, 어떤 텍스트가 들었는지,
            // 합쳐진 열이나 행이 있는지 알기 위함입니다.
            Document doc = Jsoup.parse(html);

            Map<String, List<List<CmHwpVO>>> tableMap = parseTable(doc);
            if (hwpFile != null) {
                // 테이블 그리기 시작.
                ...
                HWPWriter.toFile(hwpFile, hwpOutputPath + nowDate + ".hwp");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return Table 구조가 담긴 Map<String, List<List<CmHwpVO>>>
     */
    private Map<String, List<List<CmHwpVO>>> parseTable(Document doc) {
        int idtable = 1;
        Map<String, List<List<CmHwpVO>>> tableMap = new HashMap<String, List<List<CmHwpVO>>>();
        Elements table = doc.select("table");
        for (Element e : table) {
            String tableID = String.format("Table%d", idtable);
            List<List<CmHwpVO>> parseData = parseList(e);
    ...
    /**
     * @param table
     * @return 정렬된 테이블 정보 List<List<CmHwpVO>>
     */
    private List<List<CmHwpVO>> parseList(Element table) {
        List<List<CmHwpVO>> returnRowDataList = new ArrayList<List<CmHwpVO>>();
        List<Map<Integer, CmHwpVO>> rowDataList = new ArrayList<Map<Integer, CmHwpVO>>();
        ...
        for (int i = 0; i < rows.size(); i++) {
            rowDataList.add(i, new HashMap<Integer, CmHwpVO>());
        }
        ...
        for (Map<Integer, CmHwpVO> m : rowDataList) {
            TreeMap<Integer, CmHwpVO> tm = new TreeMap<Integer, CmHwpVO>(m);
            Iterator<Integer> iteratorKey = tm.keySet().iterator();
            List<CmHwpVO> subList = new ArrayList<CmHwpVO>();
    ...
    public void makeTable(List<List<CmHwpVO>> rowList) {
    ...
    private void addCell(ControlTable table, List<List<CmHwpVO>> rowList) {
        for (int i = 0; i < rowList.size(); i++) {
            // ControlTable에 Row를 추가합니다.
            Row row = table.addNewRow();

            // 이전에 입력한 셀 정보 리스트에서, Row 정보를 가져옵니다.
            List<CmHwpVO> cellList = rowList.get(i);
            for (int j = 0; j < cellList.size(); j++) {
                // Row의 Cell 리스트를 가져옵니다.
                CmHwpVO tableCell = cellList.get(j);
                ...
                setCellStyle(hwpFile, cell, "FF0000", false);
