...
public class AcsService extends EgovAbstractServiceImpl  {
    ...
    private final String sqlNs = "com.standard.mapper.basic.AcsLogMngMapper.";  
	...
	public void addList(AcsVO searchVO, Model model) throws Exception{
		...
		if ("03".equals(searchVO.getSchEtc01())) {
			// 과다접속 로그
			AcsVO rtnVo = (AcsVO)defaultDao.selectOne(sqlNs+"tmchSelectCount" , searchVO);
		    paginationInfo.setTotalRecordCount( Integer.parseInt(rtnVo.getLogCnt())); 
		    
		    resultList = (List<AcsVO>) defaultDao.selectList(sqlNs+"tmchSelectList", searchVO); 
		} else {
			
			AcsVO rtnVo = (AcsVO)defaultDao.selectOne(sqlNs+"selectCount", searchVO); 
			paginationInfo.setTotalRecordCount(Integer.parseInt(rtnVo.getAcsLogCnt()));
			
			resultList = (List<AcsVO>)defaultDao.selectList(sqlNs+"selectList", searchVO); 
		}
	...
	/*public int selectCount(AcsVO vo) {
		AcsVO rtnVo = (AcsVO)defaultDao.selectOne(sqlNs+"selectCount", vo);
	...
	/*public List<AcsVO> selectList(AcsVO vo) {
		return (List<AcsVO>)defaultDao.selectList(sqlNs+"selectList", vo);
		// return acsLogMngMapper.selectList(vo);
	};*/

	// 과다접속 로그 건수 조회
	/*public int tmchSelectCount(AcsVO vo) {
		AcsVO rtnVo = (AcsVO)defaultDao.selectOne(sqlNs+"tmchSelectCount" , vo);
		return Integer.parseInt(rtnVo.getLogCnt()); 
	};*/

	// 과다접속 로그 목록 조회
	/*public List<AcsVO> tmchSelectList(AcsVO vo) {
		return (List<AcsVO>) defaultDao.selectList(sqlNs+"tmchSelectList", vo); 
	};*/

	// 시간별 접속수(현재날짜)
	public List<CommonMap> selectAcsTime() { 
		return basicDao.selectList(sqlNs+"selectAcsTime"); 
	}
}
