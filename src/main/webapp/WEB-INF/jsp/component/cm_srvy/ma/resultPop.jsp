...
<%-- 데이터 유무 확인 --%>
var ansCnt = 0;
<c:forEach items="${qstStat.sctnList }" var="sctn">
	<c:forEach items="${sctn.qstList }" var="qst">
...
<%--엑셀다운로드 --%>
const fncExportExcelFile = function(gubun){
    $("#excelDivn").val(gubun);
    $("#defaultFrm").attr({"action" : '<c:out value="${pageContext.request.contextPath}"/>'+ "/ma/us/srvy/excelDownload.do", "method" : "post", "target" : "_self", "onsubmit" : ""}).submit();
}
...
$(document).ready(function(){
    ...
    $(".btn_excel").on("click", function () {
    	if(ansCnt > 0){
	    	fncExportExcelFile($(this).data("type"));    		
    	}else{
    		alert("출력가능한 데이터가 없습니다.")
    	}
    });
});
</script>
		                            ...
		                            <table class="board_write">
		                                ...
		                                <tbody>
			                                <tr>
			                                    ...
			                                    <td colspan="3"><c:out value="${not empty qst.srvyQstTitl ? qst.srvyQstTitl : '-'}"/></td>
			                                </tr>
			                                <tr>
			                                    <th scope="row">항목설명</th>
			                                    <td colspan="3"><c:out value="${not empty qst.srvyQstCtt ? qst.srvyQstCtt : '-'}"/></td>
			                                </tr>
			                                <tr>
			                                    <th scope="row">유형</th>
    ...
    <div class="btn_area right">
        	<button type="button" class="btn btn_excel" data-type="qst">항목별 엑셀</button>
        	<button type="button" class="btn btn_excel" data-type="rply">참여자별 엑셀</button>
...
<script>
    ...
    <c:forEach var="sec" items="${qstStat.sctnList}" varStatus="statusSec">
	    <c:forEach var="qstScript" items="${sec.qstList}" varStatus="qstScriptStatus">
		    <c:if test="${qstScript.srvyAnsCgVal eq '3' || qstScript.srvyAnsCgVal eq '4' || qstScript.srvyAnsCgVal eq '6' || qstScript.srvyAnsCgVal eq '7' || qstScript.srvyAnsCgVal eq '8' || qstScript.srvyAnsCgVal eq '9' }">
		    Highcharts.chart('chart_${qstScript.srvyQstSerno}', {
		        ...
		        series: [{
		            ...
		            data: [
		            	 <c:choose>
			        		<c:when test="${qstScript.srvyAnsCgVal eq '7'}">
