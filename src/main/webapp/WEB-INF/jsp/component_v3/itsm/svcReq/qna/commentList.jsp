...
<div class="reply_box">
   <div class="clear">
	   <span class="reply_tit">답변 <span class="reply_num">(${fn:length(commentList)})</span></span>
	   <span class="reply_bytes"><strong class="viewByte">0</strong>/1500 bytes (한글 500자)</span>
   </div>
   <textarea id="cmntCn" name="cmntCn" title="답변을 입력하세요." onkeyup="viewDisplay(this)" maxlength="500" style="resize: none;"></textarea>
				            ...
				            <div class="reply_cont re_cont">
					            ...
					            <div id="atch_${result.cmntSn}_FileUpload"></div>
				             	<c:if test="${not empty result.cmntAtchFileId && result.delAtchCnt + 0 gt 0 }">
					        		<script>
					        			$("#atch_${result.cmntSn}_FileUpload").html(setFileList("${result.cmntAtchFileId}", "comAtch_${result.cmntSn }_FileId", "view"));
					        			//$("#atchFileUpload_${result.cmntSn}").find(".atchFileNotExist").remove();
					        		</script>
				        		</c:if>
