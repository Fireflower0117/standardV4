	...
	<c:if test="${sessionScope.SESSION_ITSM_WRITE_BTN_KEY}">
		...
		const fncUrlSet = function(){
			...
			if($("#menuLvl").val() != "0" && menuCd != ""){
				baseUrl = "/itsm" + $("#menu_url_addr_hidden").val();
			<%-- 메뉴코드 중복검사한 경우만 세팅--%>
			}else if(menuCd != ""){
				baseUrl = "/itsm/";
			}else{
...
<form:form modelAttribute="menuVO" id="defaultFrm" name="defaultFrm" method="post" onsubmit="return false;"> 
	...
	<div class="board_top">
		<div class="board_left">
			<i class="ic power"></i>
			<c:out value="${sessionScope.SESSION_ITSM_WRITE_BTN_KEY ? '등록/수정' : '상세보기'}"/>
		</div>
	...
	<table class="board_write">
		...
		<c:choose>
			<c:when test="${sessionScope.SESSION_ITSM_WRITE_BTN_KEY}">
				<tbody> 
					<tr>
	...
	<c:if test="${sessionScope.SESSION_ITSM_WRITE_BTN_KEY}">
		<div class="btn_area">
			<a href="javascript:void(0)" id="btn_menu_submit" class="btn btn_mdl btn_write"><c:out value="${empty menuVO.menuCd ? '등록' : '수정'}"/></a>
			<c:if test="${not empty menuVO.menuCd}">
				<a href="javascript:void(0)" id="btn_menu_delete" class="btn btn_mdl btn_del">삭제</a>
			</c:if>
		</div>	
	</c:if>
</form:form>
