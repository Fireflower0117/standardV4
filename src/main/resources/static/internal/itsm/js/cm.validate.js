...
*/
function wrestGetRegex(patternVal)
{
	let max = new Map();
...
{	
	...
	if(fld.type == "radio" || fld.type == "checkbox"){
		...
		// numDotOnly, numYearMonthOnly - 년도 패턴 체크
		} else if ($('#'+id).hasClass("numDotOnly") || $('#'+id).hasClass("numYearMonthOnly")) {
			
			var regex = RegExp(/^\d{4}.(0[1-9]|1[012]).(0[1-9]|[12][0-9]|3[01])$/);
			if(!regex.test(fld.value)) {
				wrestMsg = wrestItemname(fld) + " : 올바른 날짜를 입력해주세요.";
				req_chk += 1;
				
				var msgHtml = '<div class="msg_div_only" style="width: 100%;"><strong id="msg_'+id+'" class="msg_only"><font color="red">&nbsp;'+wrestMsg+'</font></strong></div>	';
				
				$("#msg_"+id).remove();
				
				if($("#"+id).parents('td').length > 0) {
					console.log('td');
					if($("#"+id).parents('td').children('.msg_div_only').length > 0) {
						$("#"+id).parents('td').children('.msg_div_only').append('<br><strong id="msg_'+id+'" class="msg_only"><font color="red">&nbsp;'+wrestMsg+'</font></strong>');
					} else {
						$("#"+id).parents('td').append(msgHtml);
					}
					
				} else if ($("#"+id).parents('div').length > 0) {
					console.log('div');
					if($("#"+id).parents('div').children('.msg_div_only').length > 0) {
						$("#"+id).parents('div').children('.msg_div_only').append('<br><strong id="msg_'+id+'" class="msg_only"><font color="red">&nbsp;'+wrestMsg+'</font></strong>');
					} else {
						$("#"+id).parents('div').append(msgHtml);
					}
					
				} else {
					$("#"+id).parent().append('<strong id="msg_'+id+'" class="msg_only"><font color="red">&nbsp;'+wrestMsg+'</font></strong>');
				}
				
			}
		
		// 시간 체크 - numTimeOnly
		} else if ($('#'+id).hasClass("numTimeOnly")) {
			
			var regex = RegExp(/^(0|0[1-9]|1[0-9]|2[0-3])$/);
			if(!regex.test(fld.value)) {
				wrestMsg = wrestItemname(fld) + " : 올바른 시간을 입력해주세요.";
				req_chk += 1;
		...
		// 분 체크 - numMinOnly
		} else if ($('#'+id).hasClass("numMinOnly")) {
			
			var regex = RegExp(/^[0-5][0-9]$/);
			if(!regex.test(fld.value)) {
				wrestMsg = wrestItemname(fld) + " : 올바른 분을 입력해주세요.";
				req_chk += 1;
		...
		// 패턴체크
			...
			/*
				...
				패턴 체크
			*/
			if (fld.getAttribute("pattern") != null){
				let regex;
				let max = new Map();
				max = wrestGetRegex(fld.getAttribute("pattern"));
				
				regex = max.get('regex');
				errMsg = max.get('errMsg');
				// wrestItemname(fld) + " : " + 
