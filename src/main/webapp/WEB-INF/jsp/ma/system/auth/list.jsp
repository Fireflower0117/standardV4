       ...
       // 화면 Load시 목록 조회
         ...
         // 검색 초기화
          ...
          $(".board_list > tbody").on("click" , (evt) => {
            ...
            /*let tarBltnbSerno = target.find("td[data-bltnbserno]").data("bltnbserno");
            if(!$.opnt.valid.isEmpty(tarBltnbSerno)){
              $.opnt.html.dynaGenHiddenForm({ formDefine : { fid: "libraryDetailForm", action: "/ma/board/libraryview.do", method: "post", isSubmit : true }
                                                 , formAttrs : { condition  : { name : "searchCondition" , val: JSON.stringify(searchConditionObj) }
                                                               , bltnbSerno : { name : "bltnbSerno"      , val: tarBltnbSerno }
                                                               , menuCd     : { name : "menuCd"          , val: "library" }
                                                               }
              });
            }*/
          });
