        	...
        	<c:choose>
				<c:when test="${fn:length(resultList) > 0 }">
					<c:forEach items="${resultList }" var="result" varStatus="status">
						<tr  onclick="javascript:fncPageBoard('view', 'view.do', '${result.rptSn}', 'rptSn');" style="cursor:pointer;">
							...
							<td><c:out value="${result.monWeek }차 주간공정"/></td>
							<td><c:out value="${result.monWeek }"/></td>
							<td><c:out value="${result.weekStartYmd } ~ ${result.weekEndYmd }"/></td>
							<td onclick="event.cancelBubble=true;" style="cursor: default">
								<a href="#" class="btn btn_down" id="btn_excel" onclick="siteReportPop('${result.rptSn}');" ><span>보고서 보기</span></a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
