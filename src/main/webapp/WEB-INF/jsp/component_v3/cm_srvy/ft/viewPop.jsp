...
<link rel="stylesheet" type="text/css" href="<c:out value='${pageContext.request.contextPath}'/>/component/cm_srvy/css/cm_srvy.css?var=1">
...
const fncGetNextSctnPage = function(hide, show, srvySctnSerno, srvyNextSctnNo) {
    ...
    for (var i = 0; i < document.srvyFrm.elements.length; i++) {
    	...
    	if($("#" + el.getAttribute("id")).attr('type') == 'radio' && $("#" + el.getAttribute("id")).attr("data-srvysctnSerno") == srvySctnSerno){
        	if($("#" + el.getAttribute("id")).prop("checked")){
        		if($("#" + el.getAttribute("id")).attr("data-srvynextsctnno") != null && $("#" + el.getAttribute("id")).attr("data-srvynextsctnno") != ''){
            		show = parseInt($("#" + el.getAttribute("id")).attr("data-srvynextsctnno")) + 1;
            	}	
        	}
        }
    }
...
const srvySubmit = function (url,srvySctnSerno) {
    ...
    var submitChk = true;
	    if (confirm("설문을 제출하시겠습니까? 설문조사는 제출 후 수정이 불가능합니다.")) {
	    	submitChk = valid(srvySctnSerno);
	        if (submitChk) {
	           fileFormSubmit("srvyFrm", "insert", function () {
	               const form = $("#srvyFrm")[0];
...
const fncTypeChkMulti = function(num, val, qst, limit) {
    ...
    $("input:checkbox[name=check_" + qst + "]:checked").change(function(){
    	if (this.getAttribute('data-ctt') == '기타' && !this.checked) {
    		 $(".rply_" + qst + "_" + num + "_etc").hide();
        }
    });
    
    $("input:checkbox[name=check_" + qst + "]:checked").each(function (idx) {
        if (this.getAttribute('data-ctt') == '기타') {
            $(".rply_" + qst + "_" + num + "_etc").show();
        } else {
            $(".rply_" + qst + "_" + num + "_etc").hide();
        }
