							...
							<td colspan="3" class="ellipsis">
								<c:choose>
									...
									<c:when test="${cmSrvyVO.srvyAnsCgVal eq 6}">
										<img src="<c:out value="${pageContext.request.contextPath}"/>/file/getImage.do?atchFileId=<c:out value="${cmSrvyVO.srvyQstItmCtt}"/>&fileSn=<c:out value="${cmSrvyVO.fileSn}"/>&fileNmPhclFileNm=<c:out value='${cmSrvyVO.fileNmPhclFileNm}'/>" alt="image" style="width: 100%;">
									</c:when>
									<c:otherwise>
